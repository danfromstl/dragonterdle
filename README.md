...
This is a local fork/recreation of [jakerella/guessle](https://github.com/jakerella/guessle), a [Wordle](https://www.powerlanguage.co.uk/wordle/) clone with a few extra features. It's essentially the same rules, but you can play more than once a day. The game uses the [Official Scrabble dictionary](https://scrabble.merriam.com) as both the selection of "solution" words, and to check guesses.


## Playing the Game

Run the app locally and open [http://localhost:3000](http://localhost:3000). No account needed.
...
Make sure you have [Node.js](https://nodejs.org) installed. The app now defaults to port `3000`, uses in-memory sessions when `REDIS_URL` is not set, and disables global stats by default for local development.
...
You can create a `.env` from `.env.example` if you want to override defaults. To use Redis-backed sessions and global stats, set `REDIS_URL` and `DISABLE_STATS=false`.
...
Open the gear/options panel in the game to change the "correct letter" and "correct place" tile colors. The selected colors are saved in browser local storage and apply to the game board, keyboard hints, help examples, and history view.
